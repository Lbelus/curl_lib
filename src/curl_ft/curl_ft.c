...
node_t* login(char* username, char* password, char* user_agent_designation, char* url)
{
    ...
    cookie_jar = get_session_cookie(user_agent, user_agent_designation, url);
    ...
    printf("cookies: %s\n", cookies_str);
    printf("payload %s\n", payload);
    cookie_jar = get_auth_cookie(user_agent, user_agent_designation, cookies_str, payload, url);
    free(cookies_str);
